...
int i_init()
{
    ...
    sockfd = i_socket(AF_INET, SOCK_DGRAM, 0);//SOCK_DGRAM数据报套接字UDP,SOCK_STREAM数据流套接字TCP,AF_INET ipv4
 
    /* initialize server address */
    memset(&server, 0, sizeof(server));
    server.sin_family = AF_INET;//ipv4
    inet_pton(AF_INET, "127.0.0.1", &server.sin_addr);//点分十进制转化为二进制整数
    server.sin_port = htons(SEVR_PORT);//host to net short
